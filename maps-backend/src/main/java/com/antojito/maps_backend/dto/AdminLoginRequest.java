...
@NoArgsConstructor
@AllArgsConstructor
@Schema(description = "Payload de login para administrador")
public class AdminLoginRequest {

    @NotBlank(message = "El mail es obligatorio")
    @Email(message = "El mail no tiene formato valido")
    @Size(max = 150, message = "El mail no puede exceder 150 caracteres")
    @Schema(description = "Mail del administrador", example = "admin@antojitosmaps.com")
    private String mail;

    @NotBlank(message = "La password es obligatoria")
    @Size(max = 255, message = "La password no puede exceder 255 caracteres")
    @Schema(description = "Password del administrador", example = "Admin2026!")
